...
6. 【独自機能】高速・多機能分類スクリプト (sort_mixamo_multi.py)

オリジナルのリポジトリには存在しない、本プロジェクト独自の完全新規プログラムです。数千個に及ぶアニメーション群を、実用的な12のカテゴリーに自動で仕分けます。

インテリジェント・キーワード・マッチング: ファイル名に含まれるキーワード（例: "walk", "sword", "dance"等）を解析し、適切なカテゴリーフォルダへ自動コピーします。

複数カテゴリーへの重複割り当て: 例えば「Sword Walk」というファイルなら、「01_移動」と「05_近接武器」の両方に自動で分類。多角的な検索性を実現しました。

自動連番付与 & カタログ生成: 各カテゴリー内で 001_ からの連番をファイル名に付与。同時に、フォルダ内の内容を一目で把握できる _Catalog.txt を自動生成します。

階層構造の自動認識: src/ 直下から実行するだけで、プロジェクトルートにある Mixamo_Full_Download を読み込み、Mixamo_Categorized へ出力するスマートなパス解決機能を搭載しています。
